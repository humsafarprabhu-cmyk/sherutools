...
const TOOLS = {
  ...
  'screenshot-beautifier': { name: 'Screenshot Beautifier', desc: 'Make screenshots look professional', cat: 'DesignApplication' },
  'subscription-tracker': { name: 'Subscription Tracker', desc: 'Track and manage all your subscriptions', cat: 'FinanceApplication' },
...
for (const [slug, tool] of Object.entries(TOOLS)) {
  ...
  if (content.includes('SoftwareAppJsonLd') || content.includes('SoftwareApplication')) continue;
  
  // Add import at top
  content = `import SoftwareAppJsonLd from '@/components/SoftwareAppJsonLd';\n` + content;
  
  // Find {children} and wrap it
  const jsonLdTag = `<SoftwareAppJsonLd name="${tool.name}" description="${tool.desc}" category="${tool.cat}" url="https://sherutools.com/${slug}" />`;
  
  if (content.includes('{children}')) {
    content = content.replace('{children}', `${jsonLdTag}\n      {children}`);
  } else {
    // Might have different pattern, try wrapping the return
    console.log(`⚠️  ${slug} - no {children} pattern found, manual check needed`);
    continue;
  }
...
console.log(`✅ Patched ${patched} existing layouts with JSON-LD`);
